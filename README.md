...
2. Поднимите только Keycloak **docker compose up keycloak-server -d**
3. Зайдите в keycloak localhost:8080 (admin / admin), перейдите в реалм internet-shop -> таб Clients.
4. Откройте поочередно клиенты main_module, main_module_m2m; для каждого перегенерируйте credentials.
5. Замените секреты в строках 48 и 55 в application.yaml главного модуля для main_module и main_module_m2m соответственно.
6. Соберите проект (**mvn clean package -DskipTests=true**),
   проверьте что архив в /target называется так же, как в Dockerfile.
7. Поднимите остальные модули: docker-compose up --build.
8. В keycloak перейдите в реалм internet-shop, создайте несколько юзеров.
9. Пользуйтесь приложением.
10. Также можно запустить тесты, например, из IDE; сначала запустите Keycloak.
...
14. Товары изначально сохранены в БД.

15. Пользователи приложения либо изначально загружены в БД.
...
19. В качестве сервера авторизации OAuth2 выбран Keycloack.
